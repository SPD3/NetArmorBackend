...
def test_add_tested_vulnerability():
    db_session = Database().get_session()
    generic_add_test(lambda: check_tested_vulnerability(db_session, MICKEY_SCAN_ID, SQLI_NAME),
                     lambda: add_mock_tested_vulnerability(db_session, MICKEY_SCAN_ID, SQLI_NAME, MICKEY_EMAIL, MICKEY_URL), 
                     lambda: check_tested_vulnerability(db_session, MINNIE_SCAN_ID, CSRF_NAME),
                     lambda: add_mock_tested_vulnerability(db_session, MINNIE_SCAN_ID, CSRF_NAME, MINNIE_EMAIL, MINNIE_URL))
    db_session.rollback()

def test_duplicate_tested_vulnerability():
    db_session = Database().get_session()
    generic_duplicate_test(db_session, 
                           lambda: add_mock_tested_vulnerability(db_session, MICKEY_SCAN_ID, SQLI_NAME), 
                           lambda: add_tested_vulnerability(db_session, MICKEY_SCAN_ID, SQLI_NAME))
    db_session.rollback()