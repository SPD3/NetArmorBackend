...
def check_resource_rating(db_session, website_owner, resource_url, rating):
    resource_ratings = db_session.query(models.ResourceRating).filter(models.ResourceRating.website_owner==website_owner
                                                        and models.ResourceRating.resource_url==resource_url).all()
    if len(resource_ratings) != 1:
        return False
    resource_rating = resource_ratings[0]
    return resource_rating.rating == rating
